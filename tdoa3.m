function [lat, lon, height,EHE,elp] = tdoa3(datetoa,foa,cnr,sats)
lat=NaN;
lon=NaN;
height=NaN;
EHE=NaN;
...
if resd>100
    ...
    if length(chn)>3
        for kk=chn
            datetoa1=datetoa;
            datetoa1{kk}=[];
            [lat, lon, height,EHE,elp,resd] = tdoa3_rpt(datetoa1,foa,cnr,sats);
            if ~isnan(lat) && resd <100
                break;
            end
        end 
        if isnan(lat)
    ...
    if any(erroneous_channel)
        
        if(length(ft)-sum(erroneous_channel)>=3)
%             'possible'
            chns=chn(erroneous_channel);
            for kk=chns
                datetoa{kk}=[];
            end
            [lat, lon, height,EHE,elp] = tdoa3_rpt(datetoa,foa,cnr,sats);
        else
%             'impossible'
            EHE=NaN;
            elp=[];
